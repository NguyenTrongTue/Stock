...
namespace Stock.BE.Host.Controllers
{
    ...
    public class MarketController : ControllerBase
    {
        ...
        [HttpGet("IndexList")]
        public async Task<IActionResult> GetIndexList([FromQuery] string exchange = "HOSE", [FromQuery] int pageIndex = 1, [FromQuery] int pageSize = 1000)
        {
            ...
            var query = $"lookupRequest.exchange={exchange}&lookupRequest.pageIndex={pageIndex}&lookupRequest.pageSize={pageSize}";
            var url = $"{_apiUrl}/api/v2/Market/IndexList?{query}";
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    // Phân tích JSON và trả về kết quả
        ...
        [HttpGet("IndexComponents")]
        public async Task<IActionResult> GetIndexComponents([FromQuery] string indexCode = "", [FromQuery] int pageIndex = 1, [FromQuery] int pageSize = 1000)
        {
            var query = $"lookupRequest.indexCode={indexCode}&lookupRequest.pageIndex={pageIndex}&lookupRequest.pageSize={pageSize}";
            var requestUrl = $"{_apiUrl}/api/v2/Market/IndexComponents?{query}";
        ...
        [HttpGet("DailyOhlc")]
        public async Task<IActionResult> GetDailyOhlc([FromQuery] string symbol = "MBB", [FromQuery] string fromDate = "04/11/2024", [FromQuery] string toDate = null, [FromQuery] int pageIndex = 1, [FromQuery] int pageSize = 1000, [FromQuery] bool ascending = true)
        {
            toDate ??= DateTime.Now.ToString("dd/MM/yyyy");
            var query = $"lookupRequest.symbol={symbol}&lookupRequest.fromDate={fromDate}&lookupRequest.toDate={toDate}&lookupRequest.pageIndex={pageIndex}&lookupRequest.pageSize={pageSize}&lookupRequest.ascending={ascending}";
            var requestUrl = $"{_apiUrl}/api/v2/Market/DailyOhlc?{query}";
        ...
        [HttpGet("IntradayOhlc")]
        public async Task<IActionResult> GetIntradayOhlc([FromQuery] string symbol = "MBB", [FromQuery] string fromDate = "15/10/2024", [FromQuery] string toDate = null, [FromQuery] int pageIndex = 1, [FromQuery] int pageSize = 1000, [FromQuery] bool ascending = false)
        {
            toDate ??= DateTime.Now.ToString("dd/MM/yyyy");
            var query = $"lookupRequest.symbol={symbol}&lookupRequest.fromDate={fromDate}&lookupRequest.toDate={toDate}&lookupRequest.pageIndex={pageIndex}&lookupRequest.pageSize={pageSize}&lookupRequest.ascending={ascending}";
            var requestUrl = $"{_apiUrl}/api/v2/Market/IntradayOhlc?{query}";
